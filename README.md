...
Cloudflare Worker，跟仓库作者的 moro-n / 备份 Worker 完全无关。零数据库、零 cron、
明文协议（HTTPS 已加密传输；攻击者拿到 Worker URL 也榨不出东西）。

部署流程见 `worker/instant-push/README.md`，或打开 Moro Settings →
Instant Push → 配置。
...
> 没有这些人，Moro 会少很多功能，也少很多能把它玩明白的人。数据库暂时停止咕咕叫以表敬意。
...
- ✅ 魔改完发到 GitHub 让别人也玩——可以，但把 `LICENSE` 和那行 `Required Notice: Copyright (c) 2024-2026 NMJ (Moro / 手抓糯米机)` 一起带上，别把我的署名给我顺走了。
- ✅ 发给朋友、同学、暗恋对象——去吧，帮我扩散。

**不能干的事**（数据库开始咕咕叫）：

- ❌ 拿去卖钱。不管是卖源码、卖成品、卖会员、卖"高级版 Moro 皮肤"，都不行。
...
协议条款写的是"软件"，但这项目里最值钱的东西其实不是代码——是 **Moro 这只猫本体**：它的人设、台词风格、说话习惯、身上带着的那点故障风味道。这是按《著作权法》单独保护的角色 IP，和代码分开算账。

项目里其他的美术、图标、UI 文案也都是我的，不过没那么较真——你魔改着玩的时候顺带着用就行。

**重点划一下**：

- ✅ **整个项目一起拿去玩 / 魔改 / fork**：Moro 跟着走，本来就该这样。
- ✅ **改完发出去给别人玩**：带着原始角色和素材 OK，署名别删。
- ❌ **把 Moro 的人设 / 台词 / 形象单独扒出来搬进你的 AI 项目当"免费角色包"派发**——这是重点防的事。
- ❌ **商用一律不行**，代码和角色同等待遇。

一句话：**整个项目拿去造随便造，把 Moro 单独薅出去当免费素材就算了。**
