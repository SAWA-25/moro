...
> ✅ **已实现并发版**：`@rei-standard/amsg-sw@2.3.0`（Gap 1 = onclose + 事务级 InvalidStateError 重开兜底，dedupe / queue / multipart 库全覆盖；Gap 2 = DELIVER ack 新增可选 `businessError` 字段，并把失败持久化到 dedupe 记录上，重复包也带 businessError）。Moro 侧已 bump 到 2.3.0、重打 bundle、`SW_VERSION` → 1.15.0，并在 `utils/instantPushClient.ts` 接入 `businessError` 让超时诊断更精确。下文保留为设计记录。

> 交接给 amsg-sw 包维护方。本文描述两个**包内**的 IndexedDB 韧性缺口，给出修复方案与验收标准。
> Moro 侧的根因（主库连接风暴）已在 Moro 仓库自行修复（见文末「分工」），这里只列需要包升级才能解决的部分。
>
> 当前 Moro 锁的版本：`@rei-standard/amsg-sw@2.2.0`、`amsg-shared@0.2.0`。
...
## Gap 2（建议修，Moro 不阻塞）：DELIVER ack 的 `ok:true` 不反映业务落库结果
...
- **对 Moro 不构成 bug**：Moro 客户端不信这个 ack——它把成功信号绑在业务侧自己 `postMessage` 的 `active-msg-received` 事件上（落库成功后才 fire），ack 的 `ok` 只用来区分超时文案。所以「ack 撒谎」在 Moro 这条链路上不会变成「假成功」。
