...
export function replyQueuedUserMessageHint(p: {
    ...
    return `[系统提示（非${p.userName}发言）: ${p.userName}刚才连续发了多条消息，系统正在让你按顺序逐条回应。你这一轮只回应下面这一条，不要提前回答后面还没轮到的消息；如果聊天记录里这条后面还有${p.userName}的新消息，把它们当成还没来得及看的下一批。系统会自己显示引用关系，你只需要直接说出角色会回的话，不要复述“你引用了/正在回复/并回复了”等说明文字。\n当前要回应的消息${timeText ? `（${timeText}）` : ''}：「${clipped}」]`;
}
