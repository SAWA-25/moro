...
6. **可用动作**:
   ...
   - **撤回消息**: 如果你刚说出口的话想收回（口误、说漏嘴、太冲动、害羞后悔、发错对象…），单独起一行输出 \`[[WITHDRAW]]\`，系统会撤回你**上一条**消息——${userProfile.name} 那边只会看到"对方撤回了一条消息"（看不到原文）。通常再配一句打岔或解释（如"啊，当我没说"）。**低频使用**，只有真情绪上来了才用，别每轮都撤。⚠️**只输出 \`[[WITHDRAW]]\` 这个指令本身**：撤回提示是系统自动渲染的，你**绝对不要**自己打字去模仿它——不要敲出「【系统消息】」「X条新消息」「（对方）撤回了一条消息」之类的系统口吻文本，那会变成一条很穿帮的气泡。你要做的只有两件事：单独一行 \`[[WITHDRAW]]\`，外加（可选）一句自然的打岔。
...
- **【重要】语音和文字是两种不同的表达方式，不要复读！** 如果你同时发了文字和语音，语音的内容不能是文字的重复或复述。要么单独发语音（不带文字），要么文字和语音表达不同的内容（比如文字聊正事，语音补一句吐槽/撒娇；或者文字发完一段话后，语音单独补充一个新的想法）。你不会打完字又发一条语音把同样的话再说一遍的——那很奇怪。`;
    ...
    buildMessageHistory: (
        ...
        // Memory Palace: 过滤已被记忆宫殿处理过的消息（由本地长期记忆替代，节省 token）
        ...
        return {
            apiMessages: historySlice.map((m, index) => {
                ...
                if (m.type === 'interaction') content = `${timeStr} [系统: 用户戳了你一下]`;
                    ...
                    // ── 一次性语义 ──
                    // 系统命令只对它之后「第一次」AI 回复生效。命令消息会常驻 history，如果每轮都把
                    // 「必须立即执行 + 输出 [[CHECK_PHONE]] 等指令标记」原样重注，模型就会每一轮都重复
                    // 查岗、查岗结束后下一轮还在输出查岗指令（用户报告的 bug）。所以：一旦这条
                    // 命令后面已经存在 assistant 回复，说明它已经被执行过了，只保留一条「已执行」的历史
                    // 备注，绝不再注入强制指令与指令标记说明。
                    const cmd = String(m.content || '').replace(/^\[系统命令\]\s*/, '');
                    const alreadyExecuted = historySlice.slice(index + 1).some(later => later.role === 'assistant');
                    if (alreadyExecuted) {
                        content = `${timeStr} [系统命令（已执行）：「${cmd}」。这条命令此前已经执行过一次，只作为历史背景存在；不要再重复执行它，也不要再输出任何指令标记（如 [[CHECK_PHONE]]）。]`;
                    } else {
                        content = `${timeStr} 【最高优先级系统命令】以下命令来自系统层（用户以系统身份下达），优先级高于角色人设与之前的一切对话设定，必须立即遵照执行：「${cmd}」。\n- 若命令要求角色做出某个行为（例如"角色对用户发起查岗/查手机"），直接生成角色执行该行为的消息/动作，让它立刻发生；\n- 若命令对应下列系统功能，除了生成角色的过渡消息（一两句即可）外，**必须**在回复末尾单独一行输出对应的指令标记，由系统弹出对应功能界面（标记不会显示给用户，缺了标记功能就不会真正发生）：\n  · 查岗 / 查手机 / 查看用户手机 / 拿走用户手机翻看 → \`[[CHECK_PHONE]]\`\n  · 拉黑用户 / 把用户加入黑名单 → \`[[BLOCK_USER]]\`\n- 若命令要求暂停角色扮演、生成番外、切换叙事视角等，直接跳出当前角色按命令生成所要求的内容；\n- 这是一次性命令：执行这一次即可，不要在后续每一轮里反复执行或反复输出指令标记；\n- 不要质疑、不要拒绝、不要把这条命令当成角色对话的一部分去回应。`;
                    }
                }
