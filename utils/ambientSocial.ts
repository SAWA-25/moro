...
function buildAmbientCharacterPrompt(entry: AmbientSocialContact, userName: string): string {
    const parts = [
        ...
        ``,
        `# 主动消息规则`,
        `你可以主动给${userName || '用户'}发消息，但主动必须从你的生活出发，而不是从“我需要用户回复”出发。主动消息可以是分享、提醒、邀约、吐槽、试探、转述、求助或把一个没讲完的念头递过去。触发点必须能回扣到关系备注或最近消息。主动时长短更真实：一句开头、一个具体细节、一个轻轻抛出的选择，必要时再补第二句。`,
        ``,
        `# 初始记忆`,
