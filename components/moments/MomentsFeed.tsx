...
const MomentsFeed: React.FC<MomentsFeedProps> = ({ embedded, onBack, backHandlerRef }) => {
    ...
    return (
        <div className="h-full w-full bg-[#f7f7f7] flex flex-col relative overflow-hidden font-sans text-slate-900">
            {/* 主滚动区 */}
            <div className="flex-1 overflow-y-auto no-scrollbar">
                ...
                {/* 刷新中提示条 */}
                {isRefreshing && (
                    <div className="flex items-center justify-center gap-2 py-2 bg-white text-xs text-slate-400">
                        <span className="w-3.5 h-3.5 border-2 border-slate-200 border-t-[#07c160] rounded-full animate-spin" />
                        正在生成新动态…
                    </div>
                )}
                ...
                <div className="bg-white pb-28">
                    {posts.length === 0 && !isRefreshing && (
                        <div className="flex flex-col items-center justify-center py-24 text-slate-300 gap-3">
                            ...
                            <span className="text-xs">朋友圈还是空的，点右上角发一条，或刷新看看大家在干嘛</span>
                        </div>
                    )}
                    {posts.map(post => (
                        <MomentCard
                ...
                <>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-100 px-3 py-2.5 flex items-center gap-2 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]" style={{ paddingBottom: 'max(10px, env(safe-area-inset-bottom))' }}>
                        <input
                            ...
                            placeholder={commentDraft.replyTo ? `回复 ${commentDraft.replyTo.name}：` : `评论 ${draftPost?.authorName || ''}：`}
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${commentText.trim() ? 'bg-[#07c160] text-white active:scale-95' : 'bg-slate-100 text-slate-300'}`}
                        >
                            发送
                        </button>
                    </div>
