...
// 选择器钩子：.moro-chat-header 顶栏 / -back 返回 / -avatar 头像 / -name 名字 / -status 状态 /
//   -buffs 情绪栏(内含 button) / -token / -trigger 小闪电 / -inputbar 输入栏 /
//   -panel 加号拉起的功能面板(内含 button) / -root 整屏。

const PRESET_STORE_KEY = 'moro_chrome_css_presets_v1';

// 丢给别的 AI 的提示词（让它按想要的风格生成整段 CSS）。
const AI_PROMPT = `你是一个 CSS 设计师。我在用一个叫 Moro 的「浏览器里的虚拟手机」聊天 App，
它允许我用一段自定义 CSS 来完全重新设计「聊天顶栏 + 输入栏」这块外壳。
这段 CSS 会被注入到聊天界面里，通过下面这些固定类名生效。请帮我写一整段 CSS，
实现我想要的风格——你有很高的自由度，不要只改颜色，可以大胆重构整个顶栏的视觉。

【可用的类名（只能用这些，别用全局选择器）】
- .moro-chat-root      整个聊天屏（最外层背景）
- .moro-chat-header    顶栏整块（已是 position: relative，可在内部绝对定位子元素）
- .moro-chat-back      左侧返回箭头按钮
- .moro-chat-avatar    角色头像（默认圆形 img，可改尺寸/形状/位置/遮罩）
- .moro-chat-name      角色名字
- .moro-chat-status    名字旁/下的在线状态区
- .moro-chat-buffs     情绪状态栏容器；其中每个情绪胶囊是 .moro-chat-buffs button
- .moro-chat-token     右上角 token 用量小标签
- .moro-chat-trigger   右侧「触发 AI」的小闪电按钮
- .moro-chat-inputbar  底部输入栏整块
- .moro-chat-panel     点「＋」拉起的功能面板（表情/动作菜单），其中按钮是 .moro-chat-panel button

【必须遵守的规范】
1. 覆盖默认样式必须加 !important（尤其 .moro-chat-buffs button 带内联样式，不加 !important 盖不掉）。
2. 只允许使用上面的 .moro-chat-* 选择器及其后代/伪元素，禁止写 body、*、div、html 这类全局选择器（会污染其它界面）。
3. 这是移动端窄屏（宽约 390px），尺寸请克制、用相对单位或小数值。
4. 顶栏顶部已自动留出状态栏安全区。装饰若要贴最顶部，用 top: calc(var(--safe-top) + 数值)。
5. 不要 display:none 掉 .moro-chat-back（否则用户无法返回），除非我明确要求。
6. 想让装饰溢出到顶栏外（如垂下的挂饰、超出的波浪），需给 .moro-chat-header 加 overflow: visible。
...
// 从一段 CSS 里尽力抠出 .moro-chat-header 的背景值，给「我的预设」生成缩略色块（抠不到则用中性灰）。
const extractSwatch = (code: string): string => {
    const block = code.match(/\.moro-chat-header\s*\{([^}]*)\}/);
...
.moro-chat-name{color:#c2587f!important;}
.moro-chat-avatar{border:2px solid #ffb8d4!important;box-shadow:0 0 0 4px rgba(255,184,212,.25)!important;}
.moro-chat-buffs button{background:#fff0f6!important;color:#d6478b!important;border-color:#ffc6df!important;}
.moro-chat-trigger{color:#e86aa6!important;}
.moro-chat-token{background:#fff0f6!important;color:#c76aa0!important;border-color:#ffd4e6!important;}`,
    },
    {
        name: '霓虹夜',
...
.moro-chat-name{color:#e9d5ff!important;text-shadow:0 0 10px rgba(192,132,252,.9);}
.moro-chat-status{color:#a78bfa!important;}
.moro-chat-back,.moro-chat-trigger{color:#67e8f9!important;}
.moro-chat-avatar{border:2px solid #67e8f9!important;box-shadow:0 0 12px rgba(103,232,249,.6)!important;}
.moro-chat-buffs button{background:rgba(103,232,249,.12)!important;color:#a5f3fc!important;border-color:rgba(103,232,249,.4)!important;}
.moro-chat-token{background:rgba(168,85,247,.15)!important;color:#d8b4fe!important;border-color:rgba(168,85,247,.4)!important;}`,
    },
    {
        name: '薄荷奶绿',
...
.moro-chat-name{color:#2f8f6b!important;}
.moro-chat-avatar{border:2px solid #8fe0bf!important;box-shadow:0 0 0 4px rgba(143,224,191,.25)!important;}
.moro-chat-buffs button{background:#e7faf0!important;color:#22936a!important;border-color:#abe6cd!important;}
.moro-chat-trigger{color:#2bb088!important;}
.moro-chat-token{background:#e7faf0!important;color:#3a9b76!important;border-color:#bdebd6!important;}`,
    },
    {
        name: '暮光紫',
...
.moro-chat-name{color:#fce7ff!important;}
.moro-chat-status{color:#d6bcfa!important;}
.moro-chat-back,.moro-chat-trigger{color:#f5d0fe!important;}
.moro-chat-avatar{border:2px solid rgba(255,255,255,.7)!important;box-shadow:0 4px 14px rgba(0,0,0,.3)!important;}
.moro-chat-buffs button{background:rgba(255,255,255,.16)!important;color:#fbe8ff!important;border-color:rgba(255,255,255,.3)!important;}
.moro-chat-token{background:rgba(255,255,255,.14)!important;color:#f0e0ff!important;border-color:rgba(255,255,255,.25)!important;}`,
    },
    {
        name: '极简白',
        swatch: 'linear-gradient(135deg,#ffffff,#f3f4f6)',
        code: `/* 极简白 */
.moro-chat-header{background:#ffffff!important;border-bottom:1px solid #eef1f5!important;box-shadow:none!important;}
.moro-chat-name{color:#1f2937!important;}
.moro-chat-avatar{border:1.5px solid #e5e7eb!important;}
.moro-chat-buffs button{background:#f5f6f8!important;color:#6b7280!important;border-color:#e5e7eb!important;}
.moro-chat-trigger{color:#6366f1!important;}
.moro-chat-token{background:#f5f6f8!important;color:#9ca3af!important;border-color:#e5e7eb!important;}`,
    },
    {
        name: '淡紫毛绒',
...
.moro-chat-avatar{
  ...
  animation:moro-float 4.5s ease-in-out infinite !important;
}
@keyframes moro-float{0%,100%{transform:translateY(0);}50%{transform:translateY(-2.5px);}}
.moro-chat-name{color:#fff !important;font-weight:700 !important;letter-spacing:.5px !important;text-shadow:0 1px 4px rgba(135,95,205,.55), 0 0 10px rgba(255,255,255,.4) !important;}
.moro-chat-name::after{content:" ✦" !important;color:#fff3ff !important;font-size:.8em !important;text-shadow:0 0 6px rgba(255,255,255,.8) !important;}
.moro-chat-status{color:#f3ebff !important;font-size:.72rem !important;text-shadow:0 1px 2px rgba(130,90,200,.4) !important;}
.moro-chat-buffs button{
...
.moro-chat-token{color:#8a6bc4 !important;background:rgba(255,255,255,.5) !important;border-radius:999px !important;padding:1px 8px !important;font-size:.66rem !important;box-shadow:inset 0 1px 2px rgba(255,255,255,.8) !important;}
.moro-chat-trigger{
  ...
  animation:moro-breathe 3.2s ease-in-out infinite !important;
}
@keyframes moro-breathe{0%,100%{box-shadow:0 0 0 2px rgba(255,255,255,.6), 0 0 12px 2px rgba(200,150,250,.55), 0 3px 8px rgba(150,100,210,.45);}50%{box-shadow:0 0 0 2px rgba(255,255,255,.7), 0 0 20px 5px rgba(210,165,255,.85), 0 3px 8px rgba(150,100,210,.45);}}
.moro-chat-inputbar{
  ...
  border-radius:22px 22px 0 0 !important;box-shadow:inset 0 2px 5px rgba(255,255,255,.9), 0 -6px 18px rgba(180,140,230,.28) !important;backdrop-filter:blur(8px) !important;
    ...
    },
    {
        ...
        code: `/* ===== 和风温泉・晨光汤屋 ===== */
.moro-chat-root{background:linear-gradient(180deg,#fdf3e7 0%, #fbe9da 45%, #f6e4ea 100%) !important;}
.moro-chat-header{
...
.moro-chat-header::before{
  ...
  filter:blur(3px);opacity:0;animation:moro-steam 7s ease-in-out infinite;
}
.moro-chat-header::after{
...
.moro-chat-back{color:#7d4a44 !important;background:rgba(255,255,255,.5) !important;border:.08rem solid rgba(122,74,68,.3) !important;border-radius:50% !important;box-shadow:inset 0 0 .35rem rgba(255,255,255,.6), 0 .1rem .25rem rgba(180,120,110,.25) !important;}
.moro-chat-avatar{width:2.6rem !important;height:2.6rem !important;border-radius:50% !important;border:.12rem solid #fff7ee !important;object-fit:cover !important;box-shadow:0 0 0 .16rem rgba(212,96,74,.55), 0 0 .8rem rgba(246,178,107,.7), inset 0 0 .4rem rgba(0,0,0,.18) !important;}
.moro-chat-name{position:relative;z-index:1;color:#5a3243 !important;font-weight:700 !important;letter-spacing:.06em !important;text-shadow:0 .06rem 0 rgba(255,255,255,.5) !important;}
.moro-chat-status{position:relative;z-index:1;color:#3f8f6a !important;font-size:.66rem !important;letter-spacing:.04em !important;}
.moro-chat-status::before{content:"";display:inline-block;width:.42rem;height:.42rem;margin-right:.3rem;border-radius:50%;vertical-align:middle;background:#5cc486;box-shadow:0 0 .35rem rgba(92,196,134,.85);animation:moro-pulse 2.6s ease-in-out infinite;}
.moro-chat-buffs{gap:.3rem !important;position:relative;z-index:1;}
.moro-chat-buffs button{background:linear-gradient(#ffffff, #fdeede) !important;color:#8a4a44 !important;border:.07rem solid rgba(122,74,68,.4) !important;border-radius:.7rem !important;font-size:.66rem !important;font-weight:600 !important;letter-spacing:.02em !important;padding:.16rem .5rem !important;box-shadow:0 .1rem .25rem rgba(180,120,110,.3), inset 0 .05rem 0 rgba(255,255,255,.8) !important;}
.moro-chat-token{color:#6a3d38 !important;background:linear-gradient(#ffffff, #fbeede) !important;border:.06rem solid rgba(122,74,68,.35) !important;border-radius:.5rem !important;font-size:.62rem !important;letter-spacing:.02em !important;box-shadow:0 .1rem .25rem rgba(180,120,110,.28) !important;}
.moro-chat-trigger{color:#fff5e8 !important;background:radial-gradient(circle at 30% 30%, #f6b26b, #e0664a 72%) !important;border:.1rem solid rgba(255,255,255,.7) !important;border-radius:50% !important;animation:moro-ember 3.2s ease-in-out infinite;}
.moro-chat-inputbar{background:linear-gradient(180deg,#fff6ea,#ffece0) !important;border-top:.12rem solid rgba(212,96,74,.4) !important;border-radius:.9rem .9rem 0 0 !important;box-shadow:0 -.3rem .7rem rgba(180,120,110,.22), inset 0 .08rem 0 rgba(255,255,255,.7) !important;}
@keyframes moro-steam{0%{opacity:0;transform:translateY(.4rem) scaleY(.9);}50%{opacity:.5;}100%{opacity:0;transform:translateY(-.5rem) scaleY(1.12);}}
@keyframes moro-pulse{0%,100%{opacity:1;transform:scale(1);}50%{opacity:.5;transform:scale(.82);}}
...
// 导出码：MOROCSS1: + base64(utf8(JSON))，方便整段复制分享/换机带走。
const encodePresets = (list: Preset[]): string => 'MOROCSS1:' + btoa(unescape(encodeURIComponent(JSON.stringify(list))));
const decodePresets = (code: string): Preset[] => {
    const body = code.trim().replace(/^MOROCSS1:/, '');
...
const ChromeCssEditor: React.FC<{ value: string; onChange: (css: string) => void }> = ({ value, onChange }) => {
    ...
    const handleImport = () => {
        ...
        const code = window.prompt('粘贴预设导出码（MOROCSS1:...）：', '')?.trim();
        if (!code) return;
        let incoming: Preset[] = [];
