...
const MANUAL_ENTRIES: ManualEntry[] = [
  ...
  },
  {
    ...
    features: [
      ...
      '右上角的添加入口也能开关用户社交圈，并选择关闭社交圈时，已接入成正式角色或群聊的社交圈 NPC 是否继续留在往来。',
    ],
  ...
  },
  {
    ...
    features: [
      ...
      '说话的样子：设置 TA 打字习惯（短句、长段或按人设随意长短）、对照翻译、旁白模式、心声手记、表情联想和表情包权限。',
  ...
  },
  {
    ...
    features: [
      '主界面保留中心关系图：你在中间，角色围绕分布，角色之间的连线可点开查看详情。',
      '点某个角色节点后，会展开 TA 的关系卡；可从关系卡里给 TA 添加已有角色或非角色 NPC。',
      '手动关系按“某角色眼里的对方”保存：首次设定会同步到对方视角，之后各自编辑只改本侧；角色间私聊仍按两人共用记录保存。',
      'AI 整理会读取角色人设、生活侧写和绑定世界书：既生成已有角色之间的关系边，也会从单个角色设定里抽取家人、旧友、上司、宿敌等 NPC 关系。',
      '点角色之间的连线可查看详情，也能进入两人私聊记录，手动生成一段角色间互动。',
      '后台与摘要设置里可控制关系私聊满多少条开始压缩、保留多少条原文。',
  ...
  },
  {
    ...
    features: [
      ...
      '角色之间的关系：按“从谁的视角写”记录 TA 眼里其他成员是谁、熟不熟、有没有过节；可先用“补全空白 / 生成”起草，再手动修改保存，这些只影响对应角色发言，不会公开进群记录。',
  ...
  },
  {
    ...
    features: [
      '配置聊天模型连接，管理多个连接方案。',
      '设置界面全屏、隐藏 Moro 顶部时间 / 电量 / 网络状态栏、锁屏密码和版本信息。',
      '导入 / 导出本地备份，恢复角色、聊天、设置和素材。',
      '在“应用更新”里检查开发者发布的新版本，查看更新内容，下载新版 APK；普通下载慢时可尝试国内线路。',
      '配置实时感知：天气、新闻热点、个人笔记、小红书 / 麦当劳等外部服务。',
      '设置主动消息、浏览器通知，并可测试连接、诊断通知订阅、重置推送状态。',
    ],
    tips: ['APK 更新需要 Android 系统确认安装；如果系统提示安装权限，请按提示允许后再继续。普通下载和国内线路拿到的是同一个版本。'],
  },
  {
    app: '拼贴册',
    en: 'Appearance',
  ...
  },
  {
    ...
    en: 'X / Twitter',
    category: 'social',
    summary: '本地 AI 生成的 X / Twitter 式时间线，公开流以陌生虚拟账号为主，角色只偶尔作为熟人账号出现。',
    features: [
      '首页分 For You / Following 浏览时间线；For You 约九成是陌生 NPC 账号，不会围绕用户当前状态生成内容。',
    ...
    tips: ['推特内容保存在本机；公开流刷新和搜索补充不读取用户当前状态，主动发推、回复、私信或转发时仍会触发角色 / NPC 互动。'],
  },
  {
    app: '桌宠',
    en: 'Desktop Pet',
    category: 'daily',
    summary: '把流浪者、纳西妲等小桌宠放进 Moro：可对话、喂食、摸摸、设提醒，也能缩成跨 App 悬浮宠。',
    features: [
      '打开后可在流浪者和纳西妲之间切换，桌宠会按动作清单随机站立、行走、挥手、睡觉或做小动作。',
      '可以直接和桌宠对话；喂食区使用内置食物包，食物会显示完整图片，投喂时会飞向桌宠，不同食物会改变饱腹和好感并触发回应。',
      '每个桌宠都有单独的「设定」输入框，可给当前桌宠追加对话风格、称呼、喜好或禁忌。',
      '摸摸会增加一点好感，并播放对应互动动作；这些互动不写入来往聊天或长期记忆。',
      '点「放到桌面」后，桌宠会缩成悬浮宠；默认只显示角色，长按宠物后才出现状态、喂食、入口、缩放、隐藏和贴边按钮。',
      '拖到左右屏幕边缘会自动贴边，也可以在设置按钮里手动贴左或贴右。',
      '悬浮设置里可以调桌宠大小和掉落速度；松开拖拽后只会向下落一小段，不会自动贴到屏幕底部。',
      '提醒区可添加一次性或每日提醒；开启通知后，Moro 页面运行时会按时弹出桌宠提醒。',
    ],
    tips: [
      '桌宠互动只影响桌宠 App 内的养成状态，不会写入聊天、记忆或角色长期上下文。',
      '浏览器完全关闭后，网页无法常驻检查提醒；重新打开 Moro 后会继续检查到期提醒。',
    ],
  },
  {
    app: '自由活动',
...
const MANUAL_DESTINATIONS: Record<string, ManualDestination> = {
  ...
  '絮语·会话列表': {
    ...
    details: [
      ...
      '想整理社交圈 NPC：用右上角添加入口里的「用户社交圈」和「隐藏已接入 NPC」开关。',
  ...
  '絮语·单聊设置': {
    ...
    details: [
      ...
      '想改“这个角色怎么说话”：看说话的样子，里面有打字习惯、翻译、旁白、心声、表情联想和表情包权限。',
  ...
  '絮语·关系网': {
    ...
    details: [
      '首页是关系图：中心是你，周围是角色；中心线表示你和角色，角色之间的线表示角色彼此关系。',
      '点角色节点会打开 TA 的关系卡；在关系卡里点“加关系”可把剪影集角色或非角色 NPC 连进来。',
      '设定关系时可以写 NPC 名字、关系名称、快捷标签和补充说明；关系是单箭头视角，首次设定同步给对方，之后各自编辑互不覆盖。',
      '点 AI 整理可以让副 API 优先、主 API 回退地重建关系边；只有一个角色时，也会根据 TA 的设定生成 NPC 关系。',
      '点角色之间的连线或关系卡里的关系条，可看关系摘要和完整私聊，也能手动生成一段互动；非角色 NPC 只记录关系，不自动生成角色间私聊。',
      '后台与摘要设置用于控制关系网私聊满多少条开始压缩、保留多少条原文。',
  ...
  '絮语·群聊设置': {
    ...
    details: [
      ...
      '“角色之间的关系”页写的是私密视角：例如 A 眼里的 B 是老同事、暧昧对象、刚认识或有旧账；可以先点“补全空白”或单条“生成”起草，再按你的设定手改保存，只有 A 发言时会参考。',
  ...
  '文具盒': {
    ...
    details: [
      ...
      '基础与安全里可以打开界面全屏，也可以藏起 Moro 自带的顶部时间 / 电量 / 网络状态栏。',
      '“应用更新”可以检查开发者发布的新版 APK，下载后按 Android 系统提示完成安装；普通线路不顺时可改点国内线路。',
  ...
  '回忆标本馆': {
    ...
    details: [
      ...
      '事件盒和月度总结适合回看阶段性关系变化。',
      '需要后台整理和向量化时，请先在「文具盒」开启并填好副 API。',
  ...
  '推特': {
    ...
    details: [
      '适合看陌生虚拟账号为主的公开时间线：碎碎念、争论、转发、投票、链接卡和日常动态里会少量穿插熟人角色。',
  ...
  '桌宠': {
    appId: AppID.DesktopPet,
    path: ['桌面', '桌宠', '对话 / 喂食 / 提醒 / 悬浮桌宠'],
    details: [
      '适合把小桌宠放进 Moro 里陪你待机：打开后先选择流浪者或纳西妲。',
      '可以直接和桌宠说话；喂食时会显示食物图片和投喂动效，之后桌宠会按食物、喜恶、饱腹和好感做出回应。桌宠对话是独立互动，不会写进来往聊天记录或长期记忆。',
      '当前桌宠的「设定」只作用于 TA 自己，可写对话风格、称呼、偏好或不要说的话；切换到另一只桌宠会显示另一份设定。',
      '喂食和摸摸会改变桌宠自己的饱腹、好感和动作反应；没有配置 API 时，也会用本地短句回应。',
      '需要跨 App 陪伴时点「放到桌面」；悬浮宠默认只显示角色，长按宠物后才出现状态栏、快速喂食、桌宠入口、缩放、隐藏和贴边按钮。',
      '悬浮宠可以拖动，拖到左右屏幕边缘会自动贴边，也可以在设置按钮里手动贴左或贴右；悬浮大小和松手掉落速度可在悬浮设置里调整。',
      '提醒需要通知权限；网页只在 Moro 页面运行时检查提醒，浏览器完全关闭后不会常驻响铃。',
    ],
  },
  '自由活动': {
