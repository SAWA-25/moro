/**
 * WorldMap — 街区天际线地图（像素瓦片，贴在手账照片框里）
 * 缩放详情面板改为拼贴纸卡风。
 */
...
const WorldMap: React.FC<{ gameState: LifeSimState }> = ({ gameState }) => {
    ...
    return (
        <div style={{ position: 'relative' }}>
            ...
            <div ref={mapRef} style={{
                ...
                {tiles ? (
                    <img src={tiles.mapBackground} className="absolute inset-0 w-full h-full"
                        style={{ imageRendering: 'pixelated', objectFit: 'cover', opacity: 0.82 }} alt="" draggable={false} />
                ) : (
                    <div className="absolute inset-0" style={{ background: '#e9e3d6' }} />
                )}
                    ...
                    {gameState.families.flatMap((famA, i) =>
                        gameState.families.slice(i + 1).map(famB => (
                            <line key={`${famA.id}-${famB.id}`}
                                ...
                                stroke="rgba(120,116,106,0.22)" strokeWidth="1" strokeDasharray="4,3"
                                strokeLinecap="round" />
                        ))
                    )}
                </svg>
                    ...
                    return (
                        <div key={family.id} className="absolute cursor-pointer"
                            style={{
                                ...
                                filter: isZoomed ? 'drop-shadow(0 0 10px rgba(150,130,90,0.5))' : 'none',
                            }}
                            onClick={() => handleFamilyClick(family.id)}>
                            {isZoomed && (
                                <div className="absolute rounded-full animate-pulse"
                                    style={{ inset: '-12px', background: 'radial-gradient(circle, rgba(180,150,90,0.25) 0%, transparent 70%)', zIndex: -1 }} />
                            )}
                            {isZoomed && members.length > 0 && (
                                <div className="absolute flex gap-0.5"
                                    style={{
                                        left: '50%', top: '68%', transform: 'translate(-50%,-50%)', zIndex: 2,
                                    ...
                                    {members.slice(0, 4).map((npc, idx) => (
                                        <span key={npc.id} title={npc.name}
                                            style={{
                                                width: 4, height: 4, display: 'inline-block',
                            ...
                            {houseSrc ? (
                                <div className="flex justify-center">
                                    <img src={houseSrc} width={36} height={36}
                                        style={{
                                            imageRendering: 'pixelated',
                                            filter: isEmpty ? 'grayscale(1) opacity(0.3)' : 'drop-shadow(0 2px 6px rgba(0,0,0,0.3))',
                                            transition: 'all 0.3s',
                                        }}
                                        alt="" draggable={false} />
                                </div>
                            ) : (
                                <div style={{ textAlign: 'center', filter: isEmpty ? 'grayscale(1) opacity(0.3)' : '' }}>
                                    <NPCAvatar name={family.name} size={28} className="rounded" />
                                </div>
                            )}
                            {members.length > 0 && !isZoomed && (
                                <div className="flex flex-wrap justify-center mt-0.5" style={{ maxWidth: '52px', fontSize: '10px', lineHeight: 1.1 }}>
                                    {members.slice(0, 4).map(npc => (
                                        <span key={npc.id} title={npc.name} style={{ filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.6))' }}>
                                            <NPCAvatar name={npc.name} size={10} className="rounded-sm" />
                                        </span>
                                    ))}
                                    {members.length > 4 && (
                                        <span style={{ color: '#eee', fontSize: '7px', textShadow: '0 1px 2px #000' }}>+{members.length - 4}</span>
                                    )}
                                </div>
                            )}
                            <div className="text-center whitespace-nowrap" style={{
                                fontSize: '7px', fontWeight: 700, color: '#f4f2ed',
                                textShadow: '0 1px 3px rgba(0,0,0,0.9), 0 0 6px rgba(80,60,30,0.4)',
                ...
                {independentNPCs.length > 0 && (
                    <div className="absolute flex flex-wrap gap-0.5 items-center justify-center"
                        ...
                        {independentNPCs.map(npc => (
                            <span key={npc.id} title={`${npc.name}（独居）`} style={{ filter: 'drop-shadow(0 1px 3px rgba(0,0,0,0.5))' }}>
                ...
                {/* 天气遮罩 */}
                <div className="absolute inset-0 pointer-events-none transition-all duration-1000" style={{ background: weatherOverlay, zIndex: 25 }} />

                {/* 雨 */}
                {(weather === 'rainy' || weather === 'stormy') && (
                    <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ zIndex: 26 }}>
                        {Array.from({ length: 8 }).map((_, i) => (
                            <div key={i} className="absolute opacity-40" style={{ left: `${i * 12}%`, top: '-5px', animation: `sjfall ${0.6 + i * 0.08}s linear infinite`, animationDelay: `${i * 0.12}s` }}>
                                <IconRaindrop size={7} />
                            </div>
                        ))}
                    </div>
                ...
                {weather === 'snowy' && (
                    <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ zIndex: 26 }}>
                        {Array.from({ length: 10 }).map((_, i) => (
                            <div key={i} className="absolute opacity-50" style={{ left: `${i * 9 + 2}%`, top: '-5px', animation: `sjfall ${1.0 + i * 0.12}s linear infinite`, animationDelay: `${i * 0.18}s` }}>
                ...
                {chaosLevel > 65 && (
                    <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ zIndex: 26 }}>
                        {[IconLightning, IconFlame, IconExplosion].map((Icon, i) => (
                            <div key={i} className="absolute animate-bounce opacity-40" style={{ left: `${18 + i * 32}%`, top: `${8 + i * 12}%`, animationDelay: `${i * 0.25}s`, animationDuration: '1.2s' }}>
                                <Icon size={14} />
                            </div>
                        ))}
                    </div>
                ...
                {zoomedFamilyId && (
                    <div className="absolute inset-0" style={{ zIndex: 5 }}
                        onClick={(e) => { if (e.target === e.currentTarget) { setZoomedFamilyId(null); setSelectedFamilyId(null); } }} />
                )}
            </div>
            ...
            {zoomedFamily && (
                <div style={{
                    borderTop: '1px dashed rgba(167,162,151,0.6)',
                    background: 'rgba(255,255,255,0.85)',
                    animation: 'sjSlideDown 0.3s ease-out',
                    overflow: 'hidden',
                }}>
                    <div style={{ padding: '9px 11px' }}>
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                                <div style={{ width: 26, height: 26, borderRadius: 7, overflow: 'hidden', padding: 2, background: '#fbfaf7', outline: '1px dashed rgba(167,162,151,0.5)', outlineOffset: -2 }}>
                                    <NPCAvatar name={zoomedFamily.name} size={22} className="rounded" />
                                </div>
                                <div>
                                    <p className="font-hand" style={{ fontSize: 14, fontWeight: 700, color: '#2b2933' }}>{zoomedFamily.name}</p>
                                    <p className="font-hand" style={{ fontSize: 11, color: '#a79c8e' }}>
                            ...
                            <button onClick={() => { setZoomedFamilyId(null); setSelectedFamilyId(null); }}
                                className="scrap-btn-paper flex items-center justify-center" style={{ width: 26, height: 26 }}>
                        ...
                        {zoomedMembers.length > 0 ? (
                            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                                {zoomedMembers.map(npc => {
                                    ...
                                    return (
                                        <div key={npc.id} className="flex-shrink-0" style={{
                                            padding: '6px 9px', minWidth: 112, borderRadius: 9,
                                            background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(236,233,226,0.95)',
                                            outline: '1px dashed rgba(167,162,151,0.34)', outlineOffset: -3,
                                        }}>
                                            <div className="flex items-center gap-1.5 mb-1">
                                                <div style={{ width: 20, height: 20, borderRadius: 5, overflow: 'hidden', border: `1px solid ${profInfo.color}40` }}>
                                                    <NPCAvatar name={npc.name} size={20} className="rounded" />
                                                </div>
                                                <div style={{ minWidth: 0 }}>
                                                    <p className="font-hand truncate" style={{ fontSize: 12, fontWeight: 700, color: '#2b2933' }}>{npc.name}</p>
                                                    <p style={{ color: profInfo.color, fontSize: 8, fontWeight: 700 }}>{profInfo.zh}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                <div className="flex-1" style={{ height: 4, background: 'rgba(120,116,106,0.1)', borderRadius: 999, overflow: 'hidden' }}>
                                                    <div style={{ height: '100%', width: `${moodNorm}%`, background: moodColor, borderRadius: 999, transition: 'all 0.3s' }} />
                                                </div>
                                                <span style={{ fontSize: 8, color: moodColor, fontWeight: 700 }}>{moodLabel}</span>
                                            </div>
                                            {(hasGrudge || hasCrush) && (
                                                <div className="flex gap-1 mt-1">
                                                    {hasGrudge && <span title="记仇中"><IconFlame size={9} /></span>}
                                                    {hasCrush && <span title="暗恋中"><IconCrush size={9} /></span>}
                                                </div>
                                            )}
                                        </div>
                                    );
                            ...
                            <p className="font-hand" style={{ color: '#a79c8e', fontSize: 12, textAlign: 'center', padding: '6px 0' }}>这栋还空着</p>
                        )}
                    </div>
                </div>
            )}
