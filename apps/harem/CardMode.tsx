...
const CardMode: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    const { characters, apiConfig, auxApiConfig, addToast } = useOS();
    ...
    if (!game) {
        return (
            <div className="relative h-full w-full flex flex-col overflow-hidden animate-fade-in" style={{ color: INK, background: PAGE_BG }}>
                <PaperBackdrop />
                <ScrapHeader title="椒房记 · 经营" en="THE PEPPER HALL" onBack={onBack} />
    ...
    return (
        <div className="relative h-full w-full flex flex-col overflow-hidden animate-fade-in" style={{ color: INK, background: PAGE_BG }}>
            ...
            <ScrapHeader title="椒房记 · 经营" en="THE PEPPER HALL" onBack={onBack} right={
