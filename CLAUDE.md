...
| **椒房记·文游模式（AI 后宫文游）** | [`docs/harem-story.md`](./docs/harem-story.md) | 椒房记两套玩法（戏单 `apps/HaremApp.tsx`）：**文游模式**＝AI 实时生成后宫恋爱剧情的互动小说（引擎 `utils/haremStory.ts`、UI `apps/harem/StoryMode.tsx`，含好感/信任/嫉妒/长期记忆/角色独立记忆/事件flag/10 种回合节奏/结局判定/多周目）；**经营模式**＝旧翻牌养成（`utils/haremGame.ts`、`apps/harem/CardMode.tsx`）。改后宫剧情生成 / 状态机 / 存档 / 结局前必读 |
